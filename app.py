...
GEMINI_MODEL = "gemini-2.5-flash"  # change here if you want a different model — see README for why this one was chosen
...
def build_review_prompt(question: str, retrieved_df: pd.DataFrame, full_df: pd.DataFrame, history=None):
    ...
    pos_keywords = extract_top_keywords(full_df.loc[full_df["sentiment"] == "Positive", "text"], top_n=8)
    neg_keywords = extract_top_keywords(full_df.loc[full_df["sentiment"] == "Negative", "text"], top_n=8)
    keyword_block = ""
    if pos_keywords:
    ...
    system_prompt = (
        ...
        "its size as the total review count. Be specific and well-organized — you "
        "have room for up to about 600 words for genuinely comprehensive questions "
        "(e.g. covering all six aspects), but stay shorter for simple questions "
        "rather than padding every answer to that length. When covering several "
        "aspects or topics in one answer, keep each one to 1-2 sentences rather "
        "than a full paragraph, so breadth doesn't cost you completeness. ALWAYS "
        "finish your answer as a complete thought; never trail off mid-sentence or "
        "mid-list — if you're running long, wrap up cleanly rather than cutting "
        "off. Use short paragraphs or bullet points so longer answers stay "
        "readable. If none of the provided data is enough to answer, say so "
        "honestly rather than guessing or inventing details. The user may ask "
        "follow-up questions that refer back to earlier parts of the conversation "
        "— use the conversation history below for that context."
    )
