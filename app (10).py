...
def translate_batch_to_english(texts: list, progress=None) -> list:
    ...
    for start in range(0, len(texts), chunk_size):
        ...
        if progress is not None:
            # Spread across the same 0.04-0.07 slice of the overall progress
            # bar that was previously just a single static point.
            progress(
                0.04 + 0.03 * (chunk_num - 1) / total_chunks,
                desc=f"Translating chunk {chunk_num} of {total_chunks} ({len(texts)} reviews total)...",
            )
