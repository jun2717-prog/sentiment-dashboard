...
def run_batch_analysis(mode, single_text, multi_text, uploaded_file, progress=gr.Progress()):
    ...
    for i, text in enumerate(df["text"]):
        progress((i + 1) / total_reviews, desc=f"Analyzing review {i + 1} of {total_reviews}...")
    ...
    pos_keywords = extract_top_keywords(df.loc[df["sentiment"] == "Positive", "text"], top_n=12)
    neg_keywords = extract_top_keywords(df.loc[df["sentiment"] == "Negative", "text"], top_n=12)
    ...
    full_display_df["text"] = full_display_df["text"].str.slice(0, 90) + full_display_df["text"].str.len().gt(90).map(
        {True: "…", False: ""}
    )
...
def generate_pdf_report(df: pd.DataFrame, figs: dict):
    ...
    pdf.set_font("Helvetica", "", 9)
    positive_examples = unique_df[unique_df["sentiment"] == "Positive"].head(3)
    ...
    pdf.set_font("Helvetica", "", 9)
    negative_examples = unique_df[unique_df["sentiment"] == "Negative"].head(3)
...
GEMINI_MODEL = "gemini-3.5-flash"  # change here if you want a different model
...
def build_review_prompt(question: str, retrieved_df: pd.DataFrame, full_df: pd.DataFrame, history=None):
    ...
    system_prompt = (
        "You are a review analysis assistant. You are given two things: (1) a dataset "
        "overview with EXACT totals for the whole dataset (use this for any question about "
        "counts, totals, or overall breakdowns), and (2) a sample of individual reviews "
        "relevant to the question (use this for questions about specific content, themes, "
        "or examples — this sample is NOT the full dataset, so never treat its size as the "
        "total review count). Be concise and specific — aim for under 120 words unless the "
        "question genuinely needs more detail, and always finish your answer completely "
        "rather than trailing off. If neither the overview nor the sample contains enough "
        "information to answer, say so honestly rather than guessing or inventing details. "
        "The user may ask follow-up questions that refer back to earlier parts of the "
        "conversation — use the conversation history below for that context."
    )

    convo_block = ""
    if history:
        recent = history[-4:]  # last ~2 exchanges, to keep the prompt from growing unbounded
...
with gr.Blocks(title="AI-powered Review Analytics Dashboard") as demo:
    gr.Markdown(
        ...
        **confidence score** — how sure the AI was about that particular
        ...
        Built for **English-language reviews**. Non-English text will still
        get a rating, but keyword extraction, aspect detection, and the AI
        summary are all tuned for English and won't work reliably otherwise.
        """
    )
    ...
    with gr.Tabs():
        with gr.Tab("Analyze"):
            ...
            results_table = gr.Dataframe(
                ...
                value=pd.DataFrame(columns=["text", "sentiment", "confidence", "date"]),
            )
