...
def translate_batch_to_english(texts: list, progress=None) -> list:
    ...
    progress: optional Gradio progress callback — if given, called between
    chunks so the UI shows real movement (e.g. "chunk 2 of 3") instead of
    sitting at one static percentage for however long translation takes."""
    ...
    chunk_size = 20
    total_chunks = -(-len(texts) // chunk_size)  # ceiling division
    max_retries = 3
    results = []
    for start in range(0, len(texts), chunk_size):
        ...
        for attempt in range(1, max_retries + 1):
            if progress is not None:
                # Spread across the same 0.04-0.07 slice of the overall
                # progress bar that was previously just a single static point.
                retry_note = f", retry {attempt - 1}" if attempt > 1 else ""
                progress(
                    0.04 + 0.03 * (chunk_num - 1) / total_chunks,
                    desc=f"Translating chunk {chunk_num} of {total_chunks} ({len(texts)} reviews total{retry_note})...",
                )
            try:
                ...
                if isinstance(parsed, list) and len(parsed) == len(chunk):
                    ...
                    results.extend(chunk)  # unexpected shape — not worth retrying, keep originals
                break  # success or non-retryable shape issue — move to next chunk
            except Exception as e:
                error_text = str(e)
                # 503/UNAVAILABLE ("high demand, try again later") is Google's
                # servers being temporarily overloaded — genuinely transient,
                # unlike a 429 quota error which retrying won't fix at all.
                is_transient = "UNAVAILABLE" in error_text or "503" in error_text
                if is_transient and attempt < max_retries:
                    wait_time = 2 * attempt  # 2s, then 4s
                    print(f"[Translation] Chunk {chunk_num} attempt {attempt} hit a transient "
                          f"error ({type(e).__name__}: {e}) — retrying in {wait_time}s...")
                    time.sleep(wait_time)
                    continue
                print(f"[Translation] Chunk {chunk_num} failed after {attempt} attempt(s): "
                      f"{type(e).__name__}: {e} — keeping original text for this chunk.")
                results.extend(chunk)  # exhausted retries, or a non-transient error — keep originals
                break
...
def run_batch_analysis(mode, single_text, multi_text, uploaded_file, progress=gr.Progress()):
    ...
    if non_english_positions:
        ...
        translated = translate_batch_to_english(originals, progress=progress)
...
def generate_pdf_report(df: pd.DataFrame, figs: dict):
    ...
    def _is_pdf_renderable(text) -> bool:
        ...
        without crashing — catches non-Latin scripts like Japanese. This
        does NOT mean the text is actually English: Spanish, German, and
        French all pass this check trivially (their accented characters
        are Latin-1 safe) even when completely untranslated. See
        _is_genuinely_translated below for the check that actually
        matters for picking readable, correctly-translated examples."""
        try:
    ...
    def _is_genuinely_translated(row) -> bool:
        """True if this row is actually English — either it never needed
        translation, or translation succeeded (text differs from
        original_text). False means translation was attempted but failed
        (e.g. the Gemini quota was exhausted at analysis time), so text
        is still in its original language — this is what actually causes
        untranslated Spanish/German/French to slip into the PDF's example
        sections, since those languages render fine (see above) but
        aren't English."""
        original = row.get("original_text")
        return pd.isna(original) or row["text"] != original
    ...
    def _pick_examples(sentiment_label):
        ...
        # Prefer examples that are both genuinely translated AND safe to
        # render; fall back progressively (translated-but-maybe-garbled,
        # then anything at all) only if a stricter pool is empty.
        translated = candidates[candidates.apply(_is_genuinely_translated, axis=1)]
        renderable = candidates[candidates["text"].apply(_is_pdf_renderable)]
        if len(translated) > 0:
            pool = translated
        elif len(renderable) > 0:
            pool = renderable
        else:
            pool = candidates
        return pool.head(3)
...
with gr.Blocks(title="AI-powered Review Analytics Dashboard") as demo:
    gr.Markdown(
        ...
        *This project was built as a personal portfolio piece to demonstrate
        end-to-end AI application development — local ML inference, an
        AI-powered chat feature, data visualization, and report generation.*
        ...
        **confidence score** — how sure the AI was about that particular
        ...
        reliably on it. Translation calls a shared external AI service, so
        during periods of high demand a review may occasionally stay
        untranslated even with a key configured (the app retries
        automatically first, then falls back gracefully) — this isn't
        specific to this app, it reflects real-time demand on that service.

        **The "Ask AI" chat runs on a free API tier with a limited daily
        quota**, so it may occasionally be slower or briefly unavailable if
        that daily limit is reached — this resets every day, and every
        other feature (analysis, charts, reports) is unaffected and keeps
        working normally regardless.

        **Dates can be in mixed formats within the same file** — e.g.
        `2026-01-05`, `01/15/2026`, and `March 3, 2026` can all appear in
        the same upload and will be read correctly.
        """
    )
