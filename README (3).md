...
4. Up to 12 retrieved reviews (change `MAX_RETRIEVED_REVIEWS` in `app.py`)
   are sent to `gemini-3.5-flash` — the model actually confirmed working
   on this project — along with your question, with instructions to
   answer only from the provided reviews. `gemini-2.5-flash` returned a
   404 "no longer available" error when tested; Google retires specific
   model versions with little warning, so if you hit that error yourself,
   check [ai.google.dev/gemini-api/docs/models](https://ai.google.dev/gemini-api/docs/models)
   for the current list rather than assuming any model name in this file
   still exists, then change `GEMINI_MODEL` in `app.py` accordingly.
...
- **Non-English reviews are excluded if translation didn't succeed.**
  If a review couldn't be translated (e.g. the Gemini quota was
  exhausted at analysis time — see "Automatic translation" above), it's
  left out of keyword extraction entirely, rather than having foreign
  function words (e.g. German "die," Spanish "que") show up as if they
  were meaningful English keywords. This is detected by comparing each
  review's `text` to its `original_text`: if they're identical, the
  review is still in its original language and gets skipped here.
